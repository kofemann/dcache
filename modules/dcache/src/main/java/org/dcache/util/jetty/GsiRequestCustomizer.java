...
public class GsiRequestCustomizer implements HttpConfiguration.Customizer {
    ...
    @Override
    public Request customize(Request request, HttpFields.Mutable fields) {
        if (request.getConnectionMetaData().getConnection()
              .getEndPoint() instanceof SslEndPoint sslEndPoint) {
    ...
    public void customize(SSLEngine sslEngine, Request request) {
        ...
        try {
            ...
            LOGGER.warn("Failed to inject delegation attributes", e);
        }
    }
