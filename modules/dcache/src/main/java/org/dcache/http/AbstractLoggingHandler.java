 ...
 *
 * Copyright (C) 2019 - 2025 Deutsches Elektronen-Synchrotron
 *
...
public abstract class AbstractLoggingHandler extends Handler.Wrapper {
    ...
    private void requestCompleted(Request request, Response response) {
    ...
    protected void describeOperation(NetLoggerBuilder log,
        ...
        log.add("request.url", request.getHttpURI());
        log.add("response.code", response.getStatus());
        log.add("response.reason", getReason(response));
        log.add("location", response.getHeaders().get("Location"));
