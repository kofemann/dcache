...
public class AuthenticationHandler extends Handler.Wrapper {
    ...
    @Override
    public boolean handle(Request request, Response responce, Callback callback)
          throws Exception {
        ...
        AuthHandlerResponse response = new AuthHandlerResponse(responce, request);
        try {
            Subject suppliedIdentity = new Subject();
            addX509ChainToSubject(request, suppliedIdentity);
            addOriginToSubject(request, suppliedIdentity);
    ...
    private void addSpnegoCredentialsToSubject(Request request, Subject subject) {
        if (_isSpnegoAuthenticationEnabled) {
            var authtenticationState = org.eclipse.jetty.server.Request.getAuthenticationState(request);
            if (authtenticationState != null) {
                var principal = authtenticationState.getUserPrincipal();
    ...
    private void addQueryBearerTokenToSubject(Request request, Subject subject)
        ...
        String[] bearerTokens = Request.extractQueryParameters(request)
              .get(BEARER_TOKEN_QUERY_KEY)
              .getValues()
              .toArray(String[]::new);
    ...
    private void addXForwardForAddresses(ImmutableList.Builder<InetAddress> addresses,
          Request request) {
        String xff = nullToEmpty(request.getHeaders().get("X-Forwarded-For"));
    ...
    private class AuthHandlerResponse extends org.eclipse.jetty.server.Response.Wrapper {

        private final boolean suppressWWWAuthenticate;

        public AuthHandlerResponse(Response response, Request request) {
            super(request, response);
            suppressWWWAuthenticate = request.getHeaders().get("Suppress-WWW-Authenticate") != null;
        }
        ...
        private void addAuthenticationChallenges(int code) {
            if (code == HttpServletResponse.SC_UNAUTHORIZED) {
                if (_isSpnegoAuthenticationEnabled) {
                    ...
                    getHeaders().add(HttpHeader.WWW_AUTHENTICATE.asString(),
                          HttpHeader.NEGOTIATE.asString());
                    getHeaders().add(HttpHeader.WWW_AUTHENTICATE.asString(),
                          "Basic realm=\"" + getRealm() + "\"");
                } else {
                    if (suppressWWWAuthenticate) {
                        getHeaders().add(HttpHeader.WWW_AUTHENTICATE.asString(), (String)null);
