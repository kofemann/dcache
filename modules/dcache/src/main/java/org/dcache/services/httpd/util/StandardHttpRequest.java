...
public class StandardHttpRequest implements HttpRequest {
    ...
    public StandardHttpRequest(Request request, Response response) throws IOException, URISyntaxException {
        ...
        out = new ContentSinkOutputStream(response);
    ...
    public String getParameter(String parameter) {
        return Request.extractQueryParameters(request).get(parameter).getValue();
    }
    ...
    public void printHttpHeader(int size) {
        if (size > 0) {
            response.getHeaders().add(HttpHeader.CONTENT_LENGTH, size);
        }
    }
