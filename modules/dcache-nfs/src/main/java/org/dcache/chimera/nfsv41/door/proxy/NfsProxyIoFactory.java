...
retry:  while (true) {
            ...
            for (netaddr4 na : nfs4DeviceAddr.nflda_multipath_ds_list[0].value) {
                ...
                if (na.na_r_netid.equals("tcp") || na.na_r_netid.equals("tcp6")) {
                    ...
                    if (!isHostLocal(address) && address.isReachable((int) timeout)) {
                        return new NfsProxyIo(poolSocketAddress, context.getRemoteSocketAddress(), inode, stateid, 0);
                    }
                }
            }
            _log.warn("Failed to connect to pool within : {}, Retrying...", connectStopwatch, toString(nfs4DeviceAddr.nflda_multipath_ds_list[0].value));
        }

        _log.error("Failed to connect to pool within {} : {}, Giving up!", connectStopwatch, toString(nfs4DeviceAddr.nflda_multipath_ds_list[0].value));
